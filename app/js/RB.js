...
RB.prototype.computeAux = function () {
	...
	for (var i = 0; i < this.particles.length; i++) {
		...
		numeric.addeq(this.P, p.getMomentum());
	}
...
RB.prototype.join = function (rb) {
	this.x = numeric.add(numeric.mul(this.x, this.getMass()), numeric.mul(rb.x, rb.getMass()));
	this.mass += rb.getMass();
	this.x = numeric.div(this.x, this.getMass());
